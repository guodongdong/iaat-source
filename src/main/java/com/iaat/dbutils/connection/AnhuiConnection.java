...
public class AnhuiConnection implements  BaseConnection{

    private static LinkedList m_notUsedConnection = new LinkedList();    
    private static HashSet m_usedUsedConnection = new HashSet();    
    private static String m_url = DataSourceConfig.getProperty("Anhui_URL");   
    private static String m_user = DataSourceConfig.getProperty("Anhui_USERNAME");    
    private static String m_password = DataSourceConfig.getProperty("Anhui_PASSWORD");
    ...
    private static  AnhuiConnection single = null;
  
    static {    
        initDriver();    
    }   
  
    private  AnhuiConnection() {    
    }   
  
    public synchronized  static AnhuiConnection getInstance() {
        if (single == null) {  
            single = new AnhuiConnection();
        }  
       return single;     
    }
    ...
    public  synchronized Connection getConnection() {   
        ...
        while (m_notUsedConnection.size() > 0) {    
            try {    
                AnhuiConnectionWrapper wrapper = (AnhuiConnectionWrapper) m_notUsedConnection.removeFirst();    
        ...
        AnhuiConnectionWrapper wrapper = null;    
        ...
        wrapper = (AnhuiConnectionWrapper) list.removeFirst();    
        m_usedUsedConnection.add(wrapper);   
    ...
    private static AnhuiConnectionWrapper getNewConnection() {    
        try {    
            Connection con = DriverManager.getConnection(m_url, m_user, m_password);    
            AnhuiConnectionWrapper wrapper = new AnhuiConnectionWrapper(con);    
            return wrapper;    
        } catch (Exception e) {    
            e.printStackTrace();    
        }    
    ...
    static synchronized void pushConnectionBackToPool(AnhuiConnectionWrapper con) {    
    ...
    public static int close() {    
        ...
        while (iterator.hasNext()) {    
            try {    
                ( (AnhuiConnectionWrapper) iterator.next()).close();    
                count++;    
            } catch (Exception e) {    
            }    
        }    
        ...
        while (iterator.hasNext()) {    
            try {    
                AnhuiConnectionWrapper wrapper = (AnhuiConnectionWrapper) iterator.next();    
                wrapper.close();    
                if (DEBUG) {    
    ...
    public synchronized static void printDebugMsg(PrintStream out) {    
        ...
        msg.append("debug message in " + AnhuiConnection.class.getName());    
        msg.append("\r\n");    
...
class AnhuiConnectionWrapper implements InvocationHandler {    
    ...
    AnhuiConnectionWrapper(Connection con) {    
    ...
    public Object invoke(Object proxy, Method m, Object[] args) throws Throwable {    
        ...
        if (CLOSE_METHOD_NAME.equals(m.getName())) {    
        	AnhuiConnection.pushConnectionBackToPool(this);    
        }    
        else {    
